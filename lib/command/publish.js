...
publish.run = function (options, callback) { console.log(options.pkg.name)
    ...
    this.attachment = init_commander(attachment, this);
    this.unpublish = init_commander(unpublish, this);
...
publish.prepublish = function(options, callback) {
    ...
    this.context.logger.verbose('{{cyan prepublish}}', pkg.name);
...
publish.check_version = function(options, info, callback) {
    ...
    if(pkg.version in info.versions){
        if(options.force){
            this.context.logger.verbose('"--force" option found, force overriding "' + full_name + '".');
        ...
        this.context.logger.info('{{cyan unpublish}}', full_name);
        ...
        this.unpublish.run({
            ...
            self.context.logger.info('{{cyan publish}}', full_name);
            console.log('options', options.pkg.name)
...
publish.update_package = function(options, info, callback) {
    ...
    this.context.logger.verbose('upload attachments:', file_name);
