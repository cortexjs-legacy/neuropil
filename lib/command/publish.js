    ...
    publish.prepublish(options, function(err, res, json) {
        ...
        async.waterfall([
            ...
            function(res, info, done) {
                publish.check_version(options, info, done, logger, db);
            }
...
publish.check_version = function(options, info, callback, logger, db) {
    ...
    if(pkg.version in info.versions){
        ...
        logger.info('{{cyan unpublish}}', full_name);
        ...
        unpublish({
            ...
            logger.info('{{cyan publish}}', full_name);
            publish(options, callback, logger, db)

        }, logger, db);
    
    }else{
        publish.update_package(options, info, callback, logger, db);
    }
};
