...
exports.run = function(options, callback) {
  ...
  this.prepublish(options, function(err, res, json) {
    if (
      ...
      // only display 'unauthorized' and 'forbidden' error
      (!err.error ||
...
exports.check_version = function(options, info, callback) {
  ...
  if (!options._override && versions_list.length) {
    ...
    var max_stable = semver.maxStable(
        versions_list.filter(function (version) {
          return semver.satisfies(version, minor_range);
        })
      );
    ...
    if (max_stable) {
      ...
      if (semver.gt(max_stable, stable)) {
        error = {
          code: 'PUBLISH_OUTDATE',
          exitcode: 171,
          message: 'A newer stable patch "' + name + '@' + max_stable + '" is already found.',
          data: {
            name: name,
            version: version,
            newer: max_stable
      ...
      } else if (max_stable === stable && (stable in versions)) {
        // conflict
        if (version === stable) {
          error = {
