...
publish.check_version = function(options, info, callback) {
    ...
    if(pkg.version in info.versions){
        ...
        unpublish.run({
            ...
            self.logger.info('{{cyan publish}}', full_name);
...
publish.update_package = function(options, info, callback) {
    ...
    attachment.run({
        ...
        sha.get(options.tar, function(err, shasum) {
            ...
            var escaped_name = self.db.escape(pkg.name);
            ...
            // pkg.dist = pkg.dist || {}
            pkg.dist = {};
            pkg.dist.tarball = self.db.resolve(escaped_name + '/-/' + escaped_name + '-' + pkg.version + '.tgz');
