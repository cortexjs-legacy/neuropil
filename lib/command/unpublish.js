...
unpublish.run = function (options, callback) {
    ...
    // never use `unpublish.context.db`
    ...
    this.context.db.get( this.context.db.escape(name), function(err, res, json) {
        ...
        if(
            ...
            self.context.logger.verbose('{{cyan unpublish}} all');
            return self.all({
                name: options.name,
                rev: json._rev
        ...
        if( version in json.versions ){
            self.context.logger.verbose(
                self.context.logger.template('{{cyan unpublish}} {{name}}@{{version}}', {
                    name: name,
                    version: version
                })
            );
...
unpublish.detach = function(options, callback) {
    ...
    this.context.db.get( this.context.db.escape(options.name), function(err, res, json) {
        ...
        self.context.logger.verbose('delete attachments', path);
