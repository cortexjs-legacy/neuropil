...
Installer.prototype._get_module_info = function(callback) {
  ...
  function finish () {
    if (!version) {
      ...
      if (self.stable) {
        ...
        message = 'No stable package matched "' + pkg_name + '".';
      }

      if (self.prerelease) {
        self.context.emit(
          'warn',
            'neuropil runs in "{{cyan ' + self.prerelease + '}}" pre-release mode,\n'
          + '   which means only stable versions and "'
          + self.prerelease + '" pre-release versions are allowed to install.'
        );
        message = 'No ' + self.prerelease + '(pre-release) package matched "' + pkg_name + '".';
      }

      return callback({
        code: 'VERSION_NOT_FOUND',
        message: message || 'No Package matched "' + pkg_name + '".',
        data: {
          name: name,
          version: _version
        },
  ...
  var pr_found = semver.maxPrerelease(matched, this.prerelease);
  ...
  if (this.save) {
    var message =
      ...
      + '      {{bold cortex install ' + name + '@' + unstable_found + ' --save}}\n\n'
...
Installer.prototype._mix = function(a, b){
  for(var k in b){
    a[k] = b[k];
  }
  return a;
...
Installer.prototype._deal_dependencies = function(callback) {
  ...
  keys.forEach(function(key){
    dependencies = self._mix(dependencies || {}, self.pkg[key]);
  });
