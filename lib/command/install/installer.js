...
Installer.prototype._get_module_info_by_range = function (callback) {
    ...
    var storage_pkg = this.storage.get_pkg(name);
    var headers = {};

    if ( storage_pkg._etag ) {
        headers['if-none-match'] = storage_pkg._etag;
    }

    this.db.get(this.db.escape(name), {
        headers: headers
        ...
        if ( res.statusCode === 304 ) {
            ...
            if ( res.headers.etag ) {
                json._etag = res.headers.etag;
            }

            self.storage.set_pkg(name, json);
        }
...
Installer.prototype._get_module_info_by_version = function (callback) {
    ...
    var storage_pkg = this.storage.get_version(name, version);
    var headers = {};
    ...
    this.db.get(this.db.escape(name) + '/' + version, {
        headers: headers
        ...
        if ( res.statusCode === 304 ) {
            ...
            self.storage.set_version(name, version, json);
        }
