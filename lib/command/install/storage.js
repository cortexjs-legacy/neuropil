...
Storage.prototype.get_pkg = function(name) {
    ...
    var file = this._get_cache_file(name);
...
Storage.prototype.set_version = function (name, version, sub_pkg) {
    ...
    // just save the sub package inside the json of the entire package
...
Storage.prototype.save = function() {
    ...
    // save the cache
    lang.each(this._data_to_save, function (pkg, name) {
        var file = self._get_cache_file(name);

        fs.write(file, JSON.stringify(pkg));
    });
...
Storage.prototype._get_cache_file = function(name) {
